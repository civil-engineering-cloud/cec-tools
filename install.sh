#!/bin/bash
# CEC Tools 主安装脚本
# 用法: curl -fsSL https://gw.cec.cc:8410/tools/install.sh | bash
...
if [[ "$SCRIPT_SOURCE" == *"localhost"* ]] || [[ "$SCRIPT_SOURCE" == *"127.0.0.1"* ]]; then
    DOWNLOAD_BASE="http://localhost:80/tools"
elif curl -m 3 -fsSL -I "http://localhost:8410/tools/install.sh" >/dev/null 2>&1; then
    DOWNLOAD_BASE="http://localhost:8410/tools"
else
    DOWNLOAD_BASE="https://gw.cec.cc:8410/tools"
fi
